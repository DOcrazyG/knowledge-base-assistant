...
async def create_permission(db: AsyncSession, permission: permission_schemas.PermissionCreate):
...
async def get_permission(db: AsyncSession, permission_id: int):
    try:
        result = await db.execute(select(permission_models.Permission).filter_by(id=permission_id))
...
async def get_permission_by_name(db: AsyncSession, name: str):
    try:
        result = await db.execute(select(permission_models.Permission).filter_by(name=name))
        return result.scalar_one_or_none()
    except Exception as exc:
...
async def update_permission(db: AsyncSession, permission_id: int, permission_update: permission_schemas.PermissionUpdate):
...
async def assign_permission_to_role(db: AsyncSession, role_id: int, permission_id: int):
    ...
    try:
        ...
        await db.rollback()
        logger.error(f"Error assigning permission to role: {exc}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
